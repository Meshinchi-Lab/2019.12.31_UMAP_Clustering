---
title: "UMAP Clustering of Ribodepleted RNA-seq For All Types"
author: "Jenny Smith"
date: "2/11/21"
...
include_samples <- sample_info %>% 
  ...
  filter(!grepl("Adult AML (Stirewalt)|Cordblood", Group)) %>% 
  filter(!grepl("AZA|PAXLWH.B|PAXLWH.[CD]", Sample)) %>% 
  
  # filter(grepl("^AML$|NBM|CD34_PB", Group), 
  filter(grepl("AML|CD34_PB|CellLine|diagnostic|NBM|Normal_PB", Time_point)) %>%
...
cc$AML_Subtype["NBM"] <- "black"
cc$AML_Subtype["CD34_PB"] <- "grey90"
cc$AML_Subtype["NUP98-NSD1"] <- "slateblue1"
cc$AML_Subtype["AML, NOS"] <- "lightsteelblue3"
cc$AML_Subtype["APL"] <- "turquoise3"
...
length(input_genes)  #6643
```

```{r}
# pca_res <- run_jackstraw(TFIDF_Matrix = log2(sf.scaled+1)[,])
# 
# # saveRDS(pca_res,"TARGET_AML_NBM_AllDifferentTypes_diagnostic_allSampleTypes_2.4.21.csv")
...
htmlwidgets::saveWidget(as_widget(scatter3d),
                        "TARGET_AML_NBM_JMML_APL_DS_TMD_Models__3Dscatter.html",
                        selfcontained = TRUE,
                        background = bgcol,
